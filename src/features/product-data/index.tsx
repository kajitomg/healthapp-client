...
const ProductData = (props:ProductDataProps) => {
  return (
    <Box display={'flex'} flexDirection={'column'} justifyContent={'center'} minHeight={'60%'}>
      <Box my={2} paddingX={2}>
        <Typography fontSize={'x-large'} fontWeight={'bolder'}>{props?.data?.name}</Typography>
      </Box>
      <Box my={2} p={1} display={'flex'} flexDirection={'column'} borderRadius={1} boxShadow={theme => theme.shadows[1]}>
